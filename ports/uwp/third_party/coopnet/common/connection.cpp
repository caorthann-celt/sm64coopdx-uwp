...
void Connection::Disconnect(bool aIntentional) {
    if (!mActive) { return; }
    LOG_INFO("[%" PRIu64 "] Connection disconnect requested intentional=%d", mId, aIntentional ? 1 : 0);
...
void Connection::Receive() {
    ...
    // Some platforms return 0 for a zero-length recv(), which looks identical to a
    // graceful disconnect below. Avoid probing the socket when no bytes are ready.
