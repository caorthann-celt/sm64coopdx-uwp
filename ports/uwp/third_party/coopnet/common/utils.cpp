...
// Convert a domain name to an in_addr.
in_addr_t GetAddrFromDomain(const std::string& domain) {
    struct in_addr parsedAddr = { 0 };
    if (inet_pton(AF_INET, domain.c_str(), &parsedAddr) == 1) {
        LOG_INFO("Resolved CoopNet host '%s' as literal IPv4", domain.c_str());
        return parsedAddr.s_addr;
    }
    ...
    if (rc != 0 || result == nullptr) {
        LOG_ERROR("Failed to resolve CoopNet host '%s': %d", domain.c_str(), rc);
        if (result != nullptr) { freeaddrinfo(result); }
        return INADDR_NONE;
    }

    struct sockaddr_in* addr = reinterpret_cast<struct sockaddr_in*>(result->ai_addr);
    in_addr_t resolved = addr->sin_addr.s_addr;
    LOG_INFO("Resolved CoopNet host '%s'", domain.c_str());
    freeaddrinfo(result);
    return resolved;
}
