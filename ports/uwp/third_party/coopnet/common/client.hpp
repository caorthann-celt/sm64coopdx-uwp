...
class Client {
    private:
        ...
        void SendPendingLobbyJoin();
    public:
        uint64_t mCurrentUserId = 0;
        uint64_t mCurrentLobbyId = 0;
        uint64_t mPendingLobbyId = 0;
        std::string mPendingLobbyPassword;
        uint64_t mPendingLobbyLastAttemptTime = 0;
        uint32_t mPendingLobbyAttempts = 0;
